...
| `XHC_HF_WRITES` | `off` | `off`: every write toward the Hub is a local `405`. `on`: commits, uploads, branches, tags and repo create/move/delete are forwarded **as the cache's HF account**, each only when the caller's rules grant `push` (and `delete`, for a destructive one) on the repository. Needs `XHC_HF_AUTH=key` and `XHC_HF_RULES=enforce`, and refuses to start without them. See [Writes to the Hub](#writes-to-the-hub-xhc_hf_writes-off-by-default) |
| `XHC_HF_WRITE_MAX_BODY` | `64M` | the largest write body (a commit, mostly) read, inspected and forwarded; larger is `413`, never forwarded uninspected |
